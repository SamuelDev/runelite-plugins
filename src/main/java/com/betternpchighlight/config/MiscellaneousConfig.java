...
public interface MiscellaneousConfig {
  ...
  @ConfigItem(position = 8, keyName = "drawBeneathPerformanceMode", name = "Draw Beneath Performance Mode", description = "Uses the convex hull instead of per-triangle projection to erase NPC models. <br>Much faster with many NPCs, at the cost of slight visual accuracy", section = miscellaneousSectionName)
  default boolean drawBeneathPerformanceMode() {
    return false;
  }

  @ConfigItem(position = 9, keyName = "renderDistance", name = "Render Distance", description = "Limits overlays to be drawn to within the chosen distance from the local player. <br>Short = 7 tiles, Medium = 11 tiles", section = miscellaneousSectionName)
  default renderDistance renderDistance() {
    return renderDistance.NONE;
  }

  @ConfigItem(position = 10, keyName = "highlightPets", name = "Highlight pets", description = "Highlights followers/pets that are in any of your lists", section = miscellaneousSectionName)
  default boolean highlightPets() {
    return false;
  }

  @ConfigItem(position = 11, keyName = "deadNpcMenuColor", name = "Dead NPC Menu Color", description = "Highlights names in right click menu entry when an NPC is dead", section = miscellaneousSectionName)
  Color deadNpcMenuColor();

  @ConfigItem(position = 12, keyName = "respawnTimer", name = "Respawn Timer", description = "Marks tile and shows timer for when a marker NPC will respawn", section = miscellaneousSectionName)
  default respawnTimerMode respawnTimer() {
    return respawnTimerMode.OFF;
  }

  @Alpha
  @ConfigItem(position = 13, keyName = "respawnTimerColor", name = "Respawn Time Color", description = "Sets the color of the text for Respawn Timer", section = miscellaneousSectionName)
  default Color respawnTimerColor() {
    return Color.WHITE;
  }

  @Alpha
  @ConfigItem(position = 14, keyName = "respawnOutlineColor", name = "Respawn Outline Color", description = "Sets the color of the tile for Respawn Timer", section = miscellaneousSectionName)
  default Color respawnOutlineColor() {
    return Color.CYAN;
  }

  @Alpha
  @ConfigItem(position = 15, keyName = "respawnFillColor", name = "Respawn Fill Color", description = "Sets the fill color of the tile for Respawn Timer", section = miscellaneousSectionName)
  default Color respawnFillColor() {
    return new Color(0, 255, 255, 20);
  }

  @Range(min = 1, max = 10)
  @ConfigItem(position = 16, keyName = "respawnTileWidth", name = "Respawn Tile Width", description = "Sets the width of the tile for Respawn Timer", section = miscellaneousSectionName)
  default int respawnTileWidth() {
    return 2;
  }

  @ConfigItem(position = 17, keyName = "displayName", name = "Display Name", description = "Shows name of NPCs in the list above them", section = miscellaneousSectionName)
  default String displayName() {
    return "";
  }

  @ConfigItem(position = 18, keyName = "fontBackground", name = "Font Background", description = "Puts an outline, shadow, or nothing behind font overlays", section = miscellaneousSectionName)
  default background fontBackground() {
    return background.SHADOW;
  }

  @ConfigItem(position = 19, keyName = "npcMinimapMode", name = "Highlight Minimap", description = "Highlights NPC on minimap and/or displays name", section = miscellaneousSectionName)
  default npcMinimapMode npcMinimapMode() {
    return npcMinimapMode.OFF;
  }

  @ConfigItem(position = 20, keyName = "debugNPC", name = "Debug NPC Info", description = "Highlights all NPCs with their Name and ID", section = miscellaneousSectionName)
